...
- Uses `native\fastoverlay.def` for JNI exports
- Outputs to `build\fastoverlay.dll`

The Maven `pom.xml` will automatically pick up `build\fastoverlay.dll` and bundle it inside the JAR.

## JNI Exports (.def File)

When using JNI, you MUST export your native functions in the `native\fastoverlay.def` file:

```def
LIBRARY fastoverlay
EXPORTS
    Java_fastoverlay_FastOverlay_doSomethingNative
```
