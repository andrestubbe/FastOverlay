...
git branch -M main
git add -A
git commit -m "feat: initial commit for FastOverlay v0.1.0"
```

### 2. Connect Remote Repository
Once the repository is created on GitHub under `andrestubbe/FastOverlay`:
```bash
git remote add origin https://github.com/andrestubbe/FastOverlay.git
...
```bash
gh repo edit andrestubbe/FastOverlay \
  --description "High-Performance, Zero-Allocation native Windows Overlay API for Java" \
...
```markdown
[![Build](https://img.shields.io/github/actions/workflow/status/andrestubbe/FastOverlay/maven.yml?branch=main)](https://github.com/andrestubbe/FastOverlay/actions)
```
